...
$(document).ready(function () {
  ...
  $("#ver").on("change", function () {
    ...
    $("#modal-overflow .uk-modal-body").html(
      agree_data[agree_data.length - idx]["post_cont"]
    );
  ...
  if (m_id == "current") {
    $("#cat1").on("change", function () {
      if (bbs_mod == "lists") {
        location.href =
          "/" + seg + "/current/" + bbs_mod + "?cat=" + $(this).val();
      }
    });
  } else if (
    m_id == "myimprovement" ||
    ...
    $("#pwForm").submit(function () {
      ...
      if (!regexp_pw.test($("#usr_pw").val())) {
        alert(
          "비밀번호 형식에 맞게 입력해 주세요(영문/숫자/특수문자 포함:8~20자)."
        );
        $("#usr_pw").focus();
        return false;
      }
  ...
  if (bbs_mod == "write" || bbs_mod == "modify" || bbs_mod == "reply") {
    ...
    $(".btn_write").on("click", function () {
      ...
      if (m_id == "precedent") {
        ...
        var post_cat =
          post_cat0 + "|" + post_cat1 + "|" + post_cat2 + "|" + post_cat3;
  ...
  $("#family_site .family_wrap>a").toggle(
    function () {
      $("#family_site .family_wrap ul").show();
      $("#family_site .family_wrap>a>img").attr(
        "src",
        "/static/svg/family_link_bullet1.svg"
      );
    },
    function () {
      ...
      $("#family_site .family_wrap>a>img").attr(
        "src",
        "/static/svg/family_link_bullet.svg"
      );
    }
  );
  ...
  $(".mtab").on("click", function () {
    ...
    $(this)
      .siblings()
      .css({ background: "#5e5e5e", cursor: "pointer" })
      .removeClass(".active");
    $(".mtab")
      .eq($(this).index())
      .css({ background: "#960000", cursor: "unset" })
      .addClass(".active");
    // content
    if (idx == 1) {
...
document.querySelector("body").addEventListener("keypress", function (e) {
  ...
  if (e.key === "Enter") {
    if (m_id == "search") {
      if (
        $(":focus").attr("id") == "top_search" ||
        $(":focus").attr("id") == "mtop_search"
      ) {
        sch_top();
      } else {
        frs_sch();
      ...
      sch_top();
    } else if ($("#dtl_opt").val() == 0 && m_id == "precedent") {
      // 판례 검색
      sch_precedent();
    } else if ($("#dtl_opt").val() == 1 && m_id == "precedent") {
      // 판례 상세검색
      sch_precedent();
    } else if ($(":focus").attr("id") == "brd_sch") {
      // 게시판 검색
      sch_simple();
...
function sch_simple() {
  ...
  if (rtn) {
    ...
    if ($("#dtl_opt").val() == 1) {
    ...
    } else if ($("#dtl_opt_sanction").val() == 1) {
      let s_sds = $("#sch_date_start").val();
      let s_sde = $("#sch_date_end").val();
      if (s_sds || s_sde) {
        if (!s_sds || !s_sde) {
      ...
      if (sub_sch == "제목") {
        location.href =
          "/" + seg + "/" + m_id + "/lists?s_subj=" + s_word + param;
      } else if (sub_sch == "내용") {
        location.href =
          "/" + seg + "/" + m_id + "/lists?s_cont=" + s_word + param;
      } else if (sub_sch == "발행기관") {
        location.href =
          "/" + seg + "/" + m_id + "/lists?post_cat=" + s_word + param;
      } else if (sub_sch == "위반법률") {
        location.href =
          "/" + seg + "/" + m_id + "/lists?sanc=" + s_word + param;
      } else if (sub_sch == "제재기관") {
        location.href =
          "/" + seg + "/" + m_id + "/lists?post_sanc=" + s_word + param;
      } else {
        location.href =
          "/" + seg + "/" + m_id + "/lists?s_word=" + s_word + param;
      }
    }
  }
  return false;
